            ...
            case .page(let appbook, let pageNumber):
                ...
                let query = """
                    ...
                    FROM (
                        ...
                        ON
                            \(Database.MultipleChoiceStudentAnswer.tableName)
                                .\(Database.MultipleChoiceStudentAnswer.Column.questionId)
                            =
                            \(Database.QuestionPageJoin.tableName)
                                .\(Database.QuestionPageJoin.Column.questionId)
                        WHERE
                            \(Database.QuestionPageJoin.tableName)
                                .\(Database.QuestionPageJoin.Column.pageNumber)
                            =
                            \(pageNumber)
                        AND
                            \(Database.QuestionPageJoin.tableName)
                                .\(Database.QuestionPageJoin.Column.appbookId)
                            =
                            \(appbook.id)
                        AND
                            \(Database.MultipleChoiceStudentAnswer.tableName)
                                .\(Database.MultipleChoiceStudentAnswer.Column.isSubmitted)
                            =
                            1
                        GROUP BY
                            \(Database.MultipleChoiceStudentAnswer.tableName)
                                .\(Database.MultipleChoiceStudentAnswer.Column.version)
                    )
                """
